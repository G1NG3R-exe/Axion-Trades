  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        <a className="brand" href="#overview" aria-label="Signal Forge home">
        ...
        <div className="top-actions">
          ...
          <button className="icon-button" type="button" aria-label="Refresh dashboard" onClick={applyRange}>
            <RefreshCw size={17} />
          </button>
      ...
      <div className="dashboard" id="overview">
        <section className="instrument-row">
          <div className="instrument-title">
            <div className="ticker-logo">A</div>
            <div>
              <div className="eyebrow-row">
                <span className="eyebrow">NASDAQ · AAPL</span>
                <span className="simulated-pill"><Database size={11} /> SYNTHETIC TRAINING TAPE</span>
              </div>
          ...
          <div className="model-stamp">
            <div className="model-orb"><BrainCircuit size={18} /></div>
            <div>
              <span>Policy model</span>
              <strong>Forge PPO · v18</strong>
            </div>
            <span className="validated"><Check size={12} /> Validated</span>
          </div>
        </section>

        <section className="backtest-controls" id="backtest">
          <div className="control-heading">
            <span className="section-icon"><CalendarRange size={17} /></span>
            <div><span>Backtest window</span><small>Daily bars · realistic fills · $1 commission</small></div>
          </div>
          ...
          <div className="capital-control"><span>Starting capital</span><strong>$10,000</strong></div>
          <button className="primary-button" type="button" onClick={applyRange} disabled={isRunning}>
            {isRunning ? <RefreshCw className="spin" size={16} /> : <Play size={15} fill="currentColor" />}
            {isRunning ? "Running…" : "Run backtest"}
          </button>
          {rangeError && <div className="range-error"><Info size={14} /> {rangeError}</div>}
        </section>

        <section className="metric-grid" aria-label="Backtest summary">
          <MetricCard
            label="Strategy return"
            value={percent(result.strategyReturn)}
            detail={`${money(result.finalValue)} final value`}
            tone={result.strategyReturn >= 0 ? "positive" : "negative"}
            icon={<TrendingUp size={16} />}
          />
          <MetricCard
            label="Buy & hold"
            value={percent(result.buyHoldReturn)}
            detail={`${percent(result.alpha)} strategy alpha`}
            tone="neutral"
          ...
          <MetricCard
            ...
            detail={`${percent(result.maxDrawdown)} max drawdown`}
            tone={result.sharpe >= 1 ? "positive" : "neutral"}
            icon={<ShieldCheck size={16} />}
          />
          <MetricCard
            label="Trade quality"
            value={`${(result.winRate * 100).toFixed(0)}%`}
            detail={`${result.trades.length} orders · ${Math.floor(result.trades.length / 2)} round trips`}
            tone={result.winRate >= 0.5 ? "positive" : "neutral"}
            icon={<Target size={16} />}
          />
        </section>
        ...
        <section className="primary-grid">
          <article className="panel chart-panel">
            <div className="panel-header">
              <div>
                <div className="panel-title"><Activity size={17} /><h2>Market replay</h2><span className="session-count">{filteredData.length} sessions</span></div>
                <p>Candles, model features, and executed trades share one timeline.</p>
              </div>
          ...
          <aside className="panel model-panel" id="model">
            <div className="panel-header compact">
              <div>
                <div className="panel-title"><BrainCircuit size={17} /><h2>AI decision</h2></div>
                <p>Latest closed session</p>
              </div>
              <span className="policy-chip">POLICY v18</span>
            </div>

            <div className={`signal-card ${result.signal.toLowerCase()}`}>
              <div className="confidence-ring" style={{ "--confidence": `${result.confidence * 3.6}deg` } as React.CSSProperties}>
                <div><strong>{result.confidence}%</strong><span>confidence</span></div>
              </div>
              <div className="signal-copy">
                <span>Current action</span>
                <strong>{result.signal}</strong>
                <small>{result.signal === "BUY" ? "Trend regime confirmed" : result.signal === "SELL" ? "Risk limit triggered" : "Waiting for clean setup"}</small>
              </div>
            </div>

            <div className="factor-heading"><span>Decision factors</span><small>signal × learned weight</small></div>
                ...
                return (
                  <div className="factor" key={factor.label}>
                    <div className="factor-label">
                      ...
                      <b className={contribution >= 0 ? "positive" : "negative"}>{contribution >= 0 ? "+" : ""}{contribution.toFixed(2)}</b>
                    </div>
                    <div className="factor-track"><i className={factor.color} style={{ width: `${Math.max(6, Math.abs(contribution) * 230)}%` }} /></div>
                    <small>{Math.round(factor.weight * 100)}% model weight</small>
                  </div>
                );
              })}
            </div>
            ...
            <div className="training-box">
              <div className="training-top">
                <span><Cpu size={15} /> Training objective</span>
                <b className={result.alpha > 0 ? "target-met" : "target-miss"}>{result.alpha > 0 ? "Target beaten" : "Below target"}</b>
              </div>
              <div className="objective-row">
                <span>Beat buy & hold</span>
                <strong>{percent(result.alpha)} alpha</strong>
              </div>
              <div className="training-progress"><i style={{ width: `${trainingProgress}%` }} /></div>
              <div className="training-meta"><span>{training ? `Searching epoch ${trainingEpoch}…` : `Best checkpoint · epoch ${trainingEpoch}`}</span><span>{trainingProgress}%</span></div>
              <button className="secondary-button full" type="button" onClick={trainModel} disabled={training}>
                {training ? <RefreshCw className="spin" size={15} /> : <Zap size={15} />}
                {training ? "Optimizing weights…" : "Retrain this window"}
              </button>
            </div>
          </aside>
        </section>
        ...
        <section className="secondary-grid">
          <article className="panel equity-panel">
            <div className="panel-header compact">
              <div>
                <div className="panel-title"><TrendingUp size={17} /><h2>Equity curve</h2></div>
                <p>Portfolio growth after simulated costs</p>
              </div>
              <span className={result.alpha >= 0 ? "alpha-badge positive" : "alpha-badge negative"}>{percent(result.alpha)} α</span>
            </div>
            <div className="equity-values">
              <div><span>Strategy</span><strong>{money(result.finalValue)}</strong></div>
              <div><span>Buy & hold</span><strong>{money(STARTING_CAPITAL * (1 + result.buyHoldReturn))}</strong></div>
            </div>
            <div className="equity-legend"><span><i className="strategy" /> Forge PPO</span><span><i className="hold" /> Buy & hold</span></div>
            <EquityChart result={result} />
          </article>

          <article className="panel paper-panel" id="paper">
            <div className="panel-header compact">
              <div>
                <div className="panel-title"><Bot size={17} /><h2>Paper trading</h2><span className="paper-badge">FAKE MONEY</span></div>
                <p>Market-clock-aware simulated execution</p>
              </div>
              <span className={paperActive ? "bot-state armed" : "bot-state"}><Radio size={12} /> {paperActive ? (marketClock.isOpen || replayMode ? "Streaming" : "Armed") : "Stopped"}</span>
            </div>

            <div className="paper-balance">
              <div><span>Paper equity</span><strong>{money(paperValue, 2)}</strong></div>
              <div className={paperPnl >= 0 ? "paper-pnl positive" : "paper-pnl negative"}><span>Total P&L</span><strong>{percent(paperPnl / PAPER_STARTING_CASH, 2)}</strong><small>{money(paperPnl, 2)}</small></div>
            </div>
            <div className="paper-stats">
              <div><span>Cash</span><strong>{money(paper.cash, 2)}</strong></div>
              <div><span>AAPL position</span><strong>{paper.shares} shares</strong></div>
              <div><span>Simulated price</span><strong>{money(paperPrice, 2)}</strong></div>
            </div>

            <div className="bot-callout">
              <div className={marketClock.isOpen ? "bot-pulse active" : "bot-pulse"}><Clock3 size={16} /></div>
              <div>
                <strong>{marketClock.isOpen ? "NYSE session is open" : marketClock.label}</strong>
                <span>{paperActive ? (marketClock.isOpen ? "Model checks for a new action every tick." : replayMode ? "Demo replay is generating simulated ticks." : "Bot will wake automatically at the opening bell.") : "Arm the bot to let it trade this fake-money account."}</span>
              </div>
            </div>
            ...
            <div className="paper-actions">
              <button className={paperActive ? "primary-button stop" : "primary-button"} type="button" onClick={() => setPaperActive((active) => !active)}>
                ...
                {paperActive ? "Pause bot" : "Arm paper bot"}
              </button>
              {!marketClock.isOpen && (
                <button className={replayMode ? "secondary-button active" : "secondary-button"} type="button" onClick={() => { setReplayMode((active) => !active); setPaperActive(true); }}>
                  <Activity size={15} /> {replayMode ? "Stop replay" : "Replay demo"}
                </button>
              )}
              <button className="icon-button" type="button" onClick={resetPaper} aria-label="Reset paper account"><RotateCcw size={16} /></button>
            </div>
          </article>
        </section>
        ...
        <section className="panel activity-panel">
          <div className="panel-header compact">
            <div>
              <div className="panel-title"><Layers3 size={17} /><h2>Execution log</h2></div>
              <p>Every decision is timestamped with its trigger and confidence.</p>
            </div>
            <span className="log-count">{result.trades.length} backtest orders</span>
          </div>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Date</th><th>Side</th><th>Fill</th><th>Shares</th><th>Notional</th><th>Realized P&L</th><th>Confidence</th><th>AI rationale</th></tr></thead>
              <tbody>
                {result.trades.slice().reverse().slice(0, 8).map((trade) => (
                  <tr key={trade.id}>
                    <td>{dateLabel(trade.date)}</td>
                    <td><span className={trade.side === "BUY" ? "side-tag buy" : "side-tag sell"}>{trade.side === "BUY" ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}{trade.side}</span></td>
                    <td>{money(trade.price, 2)}</td>
                    <td>{trade.shares}</td>
                    <td>{money(trade.value)}</td>
                    <td className={trade.pnl === null ? "muted-cell" : trade.pnl >= 0 ? "positive-cell" : "negative-cell"}>{trade.pnl === null ? "Open leg" : money(trade.pnl)}</td>
                    <td><span className="confidence-cell"><i style={{ width: `${trade.confidence}%` }} />{trade.confidence}%</span></td>
                    <td>{trade.reason}</td>
                  </tr>
                ))}
                {result.trades.length === 0 && <tr><td colSpan={8} className="empty-row">No trades in this window. Try a longer range or retrain the model.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
        ...
        <footer className="app-footer">
          <div><ShieldCheck size={14} /> Research sandbox only. No real orders, brokerage connection, or guaranteed performance.</div>
          <span>AAPL training tape · deterministic simulated data · local paper account</span>
        </footer>
      </div>
