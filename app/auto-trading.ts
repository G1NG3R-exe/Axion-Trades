...
function enrichBars(raw: RawBar[]): LiveBar[] {
  ...
  return raw.map((bar, index) => {
    ...
    return { ...bar, ema9: fast, ema21: slow, ema50: medium, rsi, upperBand: mean + deviation * 2, lowerBand: mean - deviation * 2, momentum: index < 6 ? 0 : bar.close / closes[index - 6] - 1, volatility, vwap: cumulativeTypicalVolume / Math.max(1, cumulativeVolume), volumeRatio: clamp(bar.volume / Math.max(1, averageVolume), 0, 3), atr: Math.max(atr, bar.close * 0.0012), macd, macdSignal, adx, directionalIndex: clamp((plusDi - minusDi) / 38, -1, 1), openingHigh, openingLow, sessionHighBefore, sessionLowBefore, priorHigh, priorLow, priorClose, rollingHigh, rollingLow, bandWidthRatio, rangeExpansion: clamp(trueRange / Math.max(0.0001, atr), 0, 4), closeLocation, bodyStrength, obv, obvSlope };
  });
}
...
async function getLivePaperStream() {
  ...
  const response = await fetch(url, { headers: { accept: "application/json", "user-agent": "AxionTrades/1.0" } }); if (!response.ok) throw new Error(`Market data provider returned ${response.status}`); const payload = await response.json() as YahooPayload; const raw = rawBarsFromYahoo(payload); if (!raw.length) throw new Error("Market data provider returned no regular-session bars"); const latestDate = raw[raw.length - 1].date; const bars = enrichBars(raw).filter((bar) => bar.date === latestDate); liveStreamCache = { expiresAt: Date.now() + 45_000, bars }; return bars;
}
